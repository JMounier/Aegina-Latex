...
Le drop d'un item correspond à son passage de l'inventaire au monde 3D. Pour cela, le joueur à la possibilité depuis son inventaire de  glisser un tas d'objets hors de l'inventaire. Il peut aussi grâce à une touche (la touche \og Q\fg par défaut), lancer l'objet sélectionné dans sa barre d'outil. Ces actions vont faire apparaître l'objet dans le monde et le lancer à quelques mètres du personnage. Dans cet état, n'importe quel joueur peut alors ramasser l'objet dès qu'il est assez proche de lui. La récupération de l'objet ce fait par des échanges d'information entre serveur et client pour garantir que seul un joueur ramasse l'objet, et qu'il ait suffisamment de place dans son inventaire pour le stocker. De plus si deux objets identiques sont assez proches alors ils vont se rassembler. Cela permet d'afficher moins d'objets et donc d'améliorer les performances du jeu. D'autre part les objets \og hors inventaire \fg{} depuis plus d'une minute disparaissent, cela permet encore une fois d'améliorer les performances et de ne pas avoir une île envahie de ressources non utilisées.
...
En plus du fait que le contexte de notre monde soit fourni, le jeu lui-même va tourner autour d'une histoire. Cependant, l'histoire n'est pas encore définie et elle n'a donc pas encore pu être implémentée dans le jeu. Il existe néanmoins un script\footnote{script fourni en anexe} (au sens théâtrale du terme) racontant le début du jeu et sa fin. Ce script contient entre autre un didacticiel devant permettre au joueur de comprendre les bases du jeu. 
...
\end{itemize}
Nous avons corrigé la totalité des bugs rapportés pour produire la version \textit{v1.1-alpha} puis grâce aux avis, nous avons ajouté et modifié quelques détails artistiques. Ces modifications ont abouti sur une version \textit{v.1.2-alpha} qui est la version présentée lors de la première soutenance.
...
Pour la première soutenance d'Aegina nous avons réussi à atteindre tous les objectifs donné dans le cahier des charges et nous avons même pris de l'avance dans certains domaines comme le personnage qui devait être finit à 30 \% mais qui est finalement achevé à 70\%. De plus, nous avons pu rajouter des éléments qui n'étaient pas précisés dans le cahier des charges comme le chat et nous avons même créé une alpha pour que le jeu soit tester par plus d'une vingtaine de personne. En somme nous somme fières de l'avancement de notre jeux et nous allons continuer à le développé pour pouvoir en être aussi fière lors de la deuxième soutenance.
