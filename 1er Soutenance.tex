...
Peu d'informations supplémentaires sont disponibles sur les cristaux. Leur composition est totalement inconnue 
et personne ne sait d'où ils viennent. Ils ne semblent pas exister depuis la création du monde car leur forme géométrique et leurs pouvoirs en font la création d'êtres conscients possédant une technologie très avancée. Mais il ne s'agit que de suppositions qui n'ont que peu d'importance pour notre histoire.
...
Le mithril, dans le monde d'Aegina, est un minerai vert clair un peu plus solide que le fer. Assez rare à la surface des îles, on en trouve toujours en petite quantité en sous-sol puisqu'il est l'un des deux minerais nécessaires à la \og survie \fg{} d'une île. Le mithril a l'étrange particularité de modifier les phénomènes étranges d'Aegina parfois en les amplifiant, d'autres fois en les stabilisant. Il est impossible de savoir comment celui-ci va réagir avant de l'utiliser.
...
Cette partie traite principalement des \textit{skybox} et de l'utilisation que nous en faisons. La \textit{skybox} est la boîte dans laquelle se déroule le jeu. Ses cotés représentent les limites visibles du monde et il est impossible d'en sortir. La \textit{skybox} permet donc de représenter le ciel ou n'importe quel autre élément de l'arrière plan.
...
Nous avons alors décidé de faire les animations de nos personnages nous-même. 
Le premier problème que nous avons rencontré est que notre personnage n'était pas accessible depuis notre version de \textit{blender}, trop récente pour le modèle 3D que nous avons. 
...
\subsubsection{Barre de sélection (Florian)}
...
\begin{figure}[h!]
\begin{center}\includegraphics[height=1.6cm]{Toolbar}
...
Comme nous l'avions spécifié dans notre cahier des charges, le monde du jeu est généré de manière aléatoire. Pour cette première soutenance, nous avons implémenté la génération aléatoire d'un \textit{chunk}, et la liaison des \textit{chunks} entre eux, pour pouvoir générer un monde différent à chaque début de partie.

Pour le moment, le monde contient quatre \textit{chunks} reliés par des ponts, chacun de ces \textit{chunks} est choisi aléatoirement parmi des \textit{chunks} préfabriqués composés d'îles, de ponts et d'ancres. Par la suite un biome sélectionné aléatoirement est appliqué sur le \textit{chunk}, ce qui détermine le type des éléments qui pourront être mis à la place des ancres. Pour finir chaque ancre est donc remplacée par un élément cohérent avec le biome. Si le biome est \textit{désert} par exemple alors une ancre peut être remplacée par un cactus, un rocher, du sable\dots{} ou rien (c'est à dire un élément vide). Cette méthode de génération implique des milliers de possibilités pour un monde composé seulement de quatre \textit{chunks}.

Nous avons donc tous les éléments pour créer des \textit{chunks}, il ne restera plus qu'à faire apparaître le monde de manière procédurale avec un agencement des \textit{chunks} correct. Puis il sera aussi nécessaire de sauvegarder le monde avec les modifications que les joueurs y ont apportées. 
...
\subsubsection{Enumération (Florian, Romain et Théo)}

Lors de la création du jeu, il nous est arrivé plusieurs fois de vouloir créer une grande liste d'éléments contenant l'ensemble des \textit{Item} existants, l'ensemble des textes avec leur traduction en anglais ou l'ensemble des éléments pouvant être générés dans le monde. Nous avons donc créé des énumérations contenant tous ces éléments et permettant d'accéder à chacun d'eux facilement. Le choix des textes affichés dans le jeu peut alors être dynamique et se faire en fonction de la langue. Ainsi tous les textes présents dans le jeu changent lors du changement de langue à l'exception des messages informatifs du chat. 


\subsubsection{Histoire (Romain et Théo)}

En plus de l'histoire propre à Aegina, une autre histoire va servir de trame à notre jeu. Cependant, cette dernière n'est pas encore définie et elle n'a donc pas encore pu être implémentée. Il existe néanmoins un script\footnote{script fourni en annexe} (au sens théâtrale du terme) racontant le début du jeu et sa fin. Ce script contient entre autre un didacticiel devant permettre au joueur de comprendre les bases du jeu. 
 
 Il est important de noter que les éléments se trouvant dans la partie Univers du jeu de ce rapport (section~\ref{univers}) ne seront pas tous directement accessibles aux joueurs lors de l'aventure. Certaines parties devront être découvertes durant l'aventure, notamment tout ce qui concerne le fonctionnement du monde d'Aegina et l'influence négative des cristaux. 

\subsection{Mars}

\subsubsection{Interaction avec l'environnement (Florian et Julien)}
...
Le \textit{nearElement} est maintenant identifié mais il est impossible pour le joueur de faire la différence entre le \textit{nearElement} et les autres éléments. Pour mettre en évidence le \textit{nearElement}, nous utilisons un \textit{shader} particulier. Les \textit{shaders} indiquent au système de rendu comment un objet doit être affiché à l'écran. Le \textit{shader} que nous utilisons ajoute un contour rouge à la silhouette de l'objet et assombrit les couleurs de l'objet. Il devient donc très facile de distinguer le \textit{nearElement} des autres objets.
...
\end{itemize}
Actuellement, il est impossible de connaître la spécialisation d'un cristal dans le jeu, mais cela n'a que peu d'importance car pour l'instant les cristaux activés ne sont différents des cristaux inactifs que par leur couleur et n'ont aucune influence sur le jeu.
...
La survie étant le thème de notre jeu, il nous fallait construire de bonnes bases pour le développement de cette dernière. Aujourd'hui les éléments clés de notre survie sont présents. Parmi ces éléments, trois sont visibles directement en jeu grâce à des barres (figure~\ref{LHT}). Deux de ces barres, l'une symbolisant la soif et l'autre la faim du personnage, se situent en haut à droite de l'écran tandis que la troisième barre, symbolisant la santé (les points de vie), se trouve au dessus de la barre de sélection.
 ...
 Quand le personnage se retrouve sans point de vie (barre de santé vide), il sentira ses forces disparaître, c'est ce que nous appelons communément la mort. Cependant, dans notre grande bonté, nous avons décidé que la mort ne serait pas tout de suite définitive et donc chaque mort sera accompagnée d'un nouveau début. Pour le moment, le joueur \textit{respawn}, il est téléporté sur la première île qu'il a traversée et ses attributs (santé, soif et faim) sont remis au maximum. Son inventaire est identique à celui qu'il avait au moment de sa mort.
 
 \begin{figure}[h!]
\begin{center}\includegraphics[width=10cm]{BarreLHT}
...
Dans cette partie, nous allons vous présenter  les différents concepts que nous implémenterons pour la soutenance \no 2 prévue entre le 2 et le 6 mai 2016. Ces ajouts seront concentrés sur le \textit{gameplay} car jusqu'à présent il y a eu de nombreux éléments ajoutés au jeu mais aucun ne permet réellement de jouer et de s'amuser.

\subsection{Les quatre grands axes}

Pour ajouter du  \textit{gameplay} à Aegina, nous allons permettre au joueur de créer des objets, ajouter des créatures pouvant interagir avec le joueur, introduire un système de succès pour suivre la progression du joueur et ajouter un mode de combat entre équipes de joueurs en mode multi-joueurs. 

\subsubsection{Création d'objet}

L'ajout de la création d'objet permettra au joueur d'accéder à une interface de création dans son inventaire lui offrant la possibilité de transformer des objets qu'il possède actuellement en d'autres objets. La liste des objets qu'il pourra créer dépendra de l'avancement du jeu mais aussi de la présence de certains ateliers dans les environs (comme une forge par exemple). Cette liste dépendra aussi d'un \textit{livre des recettes}. Une fois que le joueur aura créé un nouvel objet  près d'un atelier spécifique, la recette de fabrication sera ajoutée à son livre et il pourra ensuite la réaliser n'importe où dans le monde (ce système est inspiré de \textit{Terraria} et \textit{Don't starve}).

\subsubsection{Ajout des créatures}

L'ajout des créatures sera une part importante de la prochaine version. Elles devront pouvoir se déplacer de manière intelligente. Certaines seront passives mais d'autres seront agressives et devront attaquer les joueurs.  Ces derniers pourront alors se battre contre ces créatures et récupérer des objets pour progresser dans leur aventure. Les créatures ne s'attaquent pas entre elles. L'ensemble des créatures possèderont une IA gérant leurs déplacements et leur comportement. Lorsqu'aucun joueur ne se trouvera dans les environs, les créatures devront pouvoir réaliser de petits déplacements sur l'île sans rentrer dans les éléments du décor ou tomber dans le vide. Ce comportement sera commun à l'ensemble des créatures qu'elles soient passives ou agressives. La présence d'un joueur dans les environs modifiera le comportement des créatures. Les modifications dépendront de l'agressivité ou non de la créature (les créatures agressives attaqueront le joueur alors que les passives fuiront par exemple).
...
Cet ajout comporte l'amélioration des cristaux pour que ceux-ci produisent un véritable effet sur l'environnement. Cette amélioration permettra de placer les cristaux au centre du \textit{gameplay} car ceux-ci deviendront essentiels à la survie du joueur. En effet les fonctionnalités de \textit{respawn} dépendront des cristaux.
...
Certains éléments déjà développés nécessitent des améliorations même s'ils n'ajoutent pas forcement du \textit{gameplay}.
...
Enfin, de nouveaux paramètres tels que la résistance et les dégâts seront à prendre en compte suite a l'implémentation des créatures hostiles et du PVP.

\subsubsection{Génération aléatoire et sauvegarde}

Les améliorations de la génération aléatoire consisteront à un ajout de chunks et d'éléments, mais aussi la création d'un système de \textit{seed} qui permettra entre autre une sauvegarde du monde simple et performante. Un \textit{seed} est un simple nombre qui est lié au monde, cela implique que deux mondes ayant le même \textit{seed} seront identiques.   

\subsubsection{Histoire}

L'amélioration de l'historie se fera en deux parties : l'écriture de l'histoire complète et l'implémentation de certains éléments de l'histoire dans le jeu. Le début de l'histoire servira de trame au didacticiel, ce qui permettra à tout nouveau joueur de découvrir le monde et les différentes interfaces mises à sa disposition.

\subsubsection{Environnement sonore}

L'ajout de sons/musiques nous permettra de dynamiser notre jeu. L'ajout des créatures amènera à l'ajout des  sons qui leur sont associés. Ces bruits rendront les créature plus impressionnantes ou au contraire attirantes, attention à ne pas se fier aux apparences qui peuvent être trompeuses...
...
Pour la première soutenance d'Aegina nous avons réussi à atteindre tous les objectifs donnés dans le cahier des charges et nous avons même pris de l'avance dans certains domaines comme le personnage qui devait être fini à 30 \% mais qui est finalement achevé à 70\%. De plus, nous avons dû ajouter des éléments qui n'étaient pas précisés dans le cahier des charges comme le chat  qui nous a été indispensable pour pouvoir tester le jeu en nous donnant la possibilité de saisir des \textit{cheatcode} par exemple. Nous avons même créé une version alpha pour que le jeu soit testé par plus d'une vingtaine de personnes. En somme, nous sommes fiers de l'avancement de notre jeu et nous allons continuer à le développer pour pouvoir en être aussi fiers lors de la deuxième soutenance.
...
\\
\begin{itemize}
\item Les cinématiques, les points importants de l'histoire, qui permettront au jeu d'avancer (cinématique sous forme de dessins qui se suivent ou de jeux de caméra dans le monde 3D)
...
\\
\textbf{En ouvrant les yeux Ille sent l'air et les doux rayons du soleil caresser son visage. Ille a l'impression d'avoir dormi très longtemps à tel point que Ille doute de savoir encore se mouvoir. Ille décide alors de se lever et de faire un peu d'exercice pour faire passer cette impression.}
\\
...
\end{itemize}
\textbf{Il se met à la recherche de nourriture et en se retournant tombe nez à nez avec un lapin. Ni une ni deux, il attaque son petit déjeuner ( attaquer en ...)}
\begin{itemize}
\item utiliser le bâton pour tuer un lapin
\end{itemize}
\textbf{Cependant, il a un vague souvenir que la viande crue n'est pas très digeste au petit déjeuner. Faute de feu, il décide quand même de manger sa viande préparée en brochette. Ouvrez l'inventaire puis créez une brochette)}
\begin{itemize}
\item créer de la nourriture à partir du bâton et du lapin tué (brochette)
\end{itemize}
\textbf{Le repas de Ille est près. Il ne lui suffit plus que de consommer sa brochette et le grondement de son ventre cessera (utiliser un consommable avec clic droit)}
\begin{itemize}
...
\textit{\underline{Personnage bloqué}}
\\
\textbf{Maintenant que Ille a trouvé de quoi survivre, il peut enfin se reposer. Jusqu'à présent  Ille n'a pensé qu'à manger mais une fois le ventre rempli il se met à penser et à se questionner \og Mais où suis-je? Quel est cet endroit ?\fg{} En effet cet archipel d'îles volantes ne lui est pas familier \og Attends !!! Des îles volantes ?? Mais ce n'est pas possible, ça ne peut pas exister\fg{} Et pourtant ce qui s'étend à perte de vue n'est pas une illusion. Ille ne se trouve plus chez lui, peut être même plus dans son monde.}  \textit{\underline{petite pause}}
\\
...
\end{itemize}
\textbf{Il ne manque pas de se demander ce qu'un pont peut bien faire dans cet archipel ayant pourtant l'air abandonné et espère que celui-ci ne va pas s'écrouler sous son poids\dots heureusement cela ne se produit pas. En traversant ce pont, Ille voit à l'horizon une sorte de reflet et intrigué décide de chercher sa source.}
\begin{itemize}
...
En s'éloignant, il se dit que récupérer du métal  va être plus difficile que de ramasser du bois ou de tuer des lapins. Il doit donc fabriquer quelque chose pour récupérer et raffiner le métal.}
\begin{itemize}
...
\\
\textbf{Ille a finalement réussi. Après tous ses efforts il a recréé un portail lui permettant de revenir chez lui. C'est en hâte qu'il efface ses derniers doutes et plonge dans le portail. Il ne sait toujours pas qui il est, mais cela n'a
