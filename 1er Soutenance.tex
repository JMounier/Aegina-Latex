...
Lors d'un hiver très rude, Ille n'eut pas d'autre choix que de voler de la nourriture dans un village avoisinant. Malheureusement, la discrétion n'étant pas son fort, il fut surpris en plein méfait et un garde tenta de l'arrêter. Ille tenta de passer le garde, mais il ne contrôla pas sa force et, en le bousculant, il le tua. Illie n'avait pas réalisé que le garde  était  extrêmement affaibli par le froid et la faim. Ille fut alors poursuivi pour meurtre et rapidement arrêté car malgré sa force Ille restait humain. Il fut condamné à  mort et envoyé dans le néant qui n'était autre qu'Aegina. C'est à son réveil que le jeu commence.

Cependant, Ille n'est pas forcement seul en Aegina. Il pourra rencontrer d'autres personnes car la mort par envoi dans le néant est une pratique assez courante. Retrouver des amis dans ce monde dangereux peut être une bonne chose car dans Aegina le nombre de vos amis fait en partie votre force.
...
\textcolor{red}{Bien relire pour vérifier que je n'ai pas introduit d'erreur}
...
La \textit{directional light} est un gameobject propose par unity. Son intérêt majeur est que l'orientation de cette lumière influence la  \textit{skybox}  du monde. De plus nous pouvons choisir la couleur et l'intensité de cette lumière. Pour ce faire nous utilisons une fonction qui convertit la longueur d'onde des rayons lumineux en couleur RGB{\textcolor{red}{\footnote{mettre une référence}} et nous représentons le spectre solaire à l'aide de la fonction mathématique suivante où $\lambda$ est la longueur et $t$ le temps : 
...
Nous avons ensuite implémenté la rotation et le déplacement de la \textit{directional light}. Ce fut assez simple car nous étudions les mouvements circulaires en cours de physique. Il nous a donc suffit de copier les équations paramétriques horaires en base cartésienne. C'est-à-dire :
$$ x(t) = R cos(wt)$$
$$ z(t)= R sin(wt)$$
\begin{center}
...
\textcolor{red}{Dans le cahier des charges vous parlez d'aléatoire, le monde généré deux fois de suite ne sera donc pas le même. S'il est généré de façon procédurale cela veut dire qu'il n'est pas prédéfini mais calculé à chaque fois (ce qui est un peu sous entendu avec le mot génération), mais en pouvant produire le même résultat. Donc en général si la génération est aléatoire elle est quasi obligatoirement procédurale. Le point à mettre en avant dans votre rapport est l'aspect aléatoire. D'après le texte suivant il semble que "seul" le biome soit choisi aléatoirement, est-ce aussi le cas des objets créés sur les ancres ?, une ancre peut-elle n'avoir aucun objet associé? Donc il faut voir clairement ce qui est "prédéfini", qui sera toujours identique d'une génération à l'autres (par exemple les chunks possibles sont prédéfinis mais leur agencement est aléatoire ?) et ce qui est généré aléatoirement (qui sera différent d'une génération à l'autre)}

Comme nous l'avions spécifié dans notre cahier des charges, le monde du jeu est généré de manière aléatoire. Pour cette première soutenance, nous avons implémenté la génération d'un \textit{chunk}, et la liaison des chunks entre eux. Nous avons donc au lancement d'une partie un monde composé de quatre chunks chacun relié par des ponts.

Nous avons créé deux chunks différents possédant chacun une île différente, cependant rien nous empêchera à l'avenir de créer des chunks ayant plusieurs îles. Sur ces îles sont posées des ancres qui correspondent a des emplacements d'objets. Cela permet lors de la génération du monde de choisir un biome aléatoirement parmi forêt, désert ou neige, puis de créer sur les ancres un objet cohérent avec le biome. Par exemple dans un biome désert nous trouverons des cactus et des rochers alors que dans un biome forêt nous trouverons des arbres et des fleurs.

Nous avons donc tous les éléments pour créer des chunks, il ne restera plus qu'a faire apparaître le monde de manière aléatoire avec un agencement des chunks correct. Puis il sera aussi nécessaire de sauvegarder le monde avec les modifications que les joueurs y ont apportées. 

\subsubsection{Drop d'Item (Florain et Julien)}

Le drop d'un item correspond à son passage de l'inventaire au monde 3D. Pour cela, le joueur a la possibilité depuis son inventaire de  glisser un tas d'objets hors de l'inventaire. Il peut aussi grâce à une touche (la touche \og Q\fg par défault), lancer l'objet sélectionné dans sa barre d'outil. Ces actions vont faire apparaître l'objet dans le monde et le lancer a quelques mètres du personnage. Dans cet état, n'importe quel joueur peut alors ramasser l'objet dès qu'il est assez proche de lui. De plus si deux objets identiques sont assez proches alors ils vont se rassembler. Cela permet d'afficher moins d'objets et donc d'améliorer les performances du jeu. D'autre part les objets \og hors inventaire \fg{} depuis plus d'une minute disparaissent, cela permet encore une fois d'améliorer les performances et de ne pas avoir une île envahie de ressources non utilisées.

\subsubsection{Bibliothèque (Florian, Romain et Théo)}
\textcolor{red}{Attention, bibliothèque est un terme qui a une signification très particulière en programmation, ça ne correspond par à ce que vous faites, il faut changer le titre de la section}
...
Il a donc fallu trouver une autre méthode pour sélectionner les objets et bien faire comprendre au joueur quel est l'objet actuellement sélectionné. Après concertation, nous avons remarqué que l'objet avec lequel le joueur veut interagir est souvent l'objet le plus proche de lui (\textit{nearElement}). Nous avons donc décidé qu'un joueur ne pourrait interagir qu'avec le \textit{nearElement} et ce en utilisant le clic droit. L'avantage de n'interagir qu'avec le \textit{nearElement}  est que celui-ci est unique. Il ne peut y avoir qu'un seul objet le plus proche et si plusieurs object sont a égale distance, alors le premier rencontré sera considéré comme \textit{nearElement}.

Maintenant que la méthode théorique est en place, il faut l'appliquer. Pour ce faire, il suffit de comparer les distances séparant le personnage des objets environnants. La sélection du \textit{nearElement} se résume donc à une recherche de minimum. Mais il faut définir l'ensemble de recherche et, pour cela, être capable de différencier les objets de l'île par exemple. La solution que nous utilisons est la suivante : nous ajoutons un \textit{tag} particulier à tous les objects avec lesquels le joueur peut interagir. Pour ne pas avoir à considérer l'ensemble des objets de monde, nous définissons une sphère d'interaction, le joueur ne peut interagir qu'avec les objets se trouvant à l'intérieur de cette sphère, les autres sont trop éloignés. L'objet le plus proche du joueur se trouve donc obligatoirement à l'intérieur de cette sphère.

Le \textit{nearElement} est maintenant identifié mais il est impossible pour le joueur de faire la différence entre le \textit{nearElement} et les autres éléments. Pour mettre en valeur le \textit{nearElement}, nous utilisons un \textit{shader} particulier. Les \textit{shaders} indiquent au système de rendu comment un objet doit être affiché à l'écran. Le \textit{shader} que nous utilisons ajoute un contour rouge à la silhouette de l'objet et assombrit les couleurs de l'objet. Il devient donc très facile de distinguer le \textit{nearElement} des autres objets.

\textcolor{red}{Il faut parler (ici ou dans une section particulière) du problème de la récupération d'un élément en mode multi-joueurs. IL faut garantir qu'un élément donné n'est récupéré que par un seul joueur. Soit votre jeu le garanti déjà, et il faut expliquer comment soit il ne le fait pas encore et il faut dire qu'il devra le garantir.}
...
\end{figure}
 ...
 Quand vous vous retrouvez sans point de vie (barre de santé vide) vous sentirez vos forces disparaître, c'est ce que nous appelons communément la mort. Cependant, dans notre grande bonté, nous avons décidé que la mort ne serait pas tout de suite définitive et donc chaque mort sera accompagnée d'un nouveau début. Pour le moment, le joueur \textit{respawn}, il est téléporté sur la première île qu'il a traversée et ses attributs (santé, soif et faim) sont remis au maximum.
...
\end{itemize}
Nous avons corrigé la totalité des bugs rapportés pour produire la version \textit{v1.1-alpha} qui est la version présentée lors de la première soutenance.
...
Dans cette partie, nous allons vous présenter  les différents concepts que nous implémenterons pour la soutenance \no 2 prévue entre le 2 et le 6 mai 2016. Ces ajouts seront concentrés sur le \textit{gameplay} car jusqu'a présent il y a eu de nombreux éléments ajoutés au jeu mais aucun ne permet réellement de jouer et de s'amuser.
...
L'amélioration des cristaux dans le mode de jeu en coopération permettra de placer les cristaux au centre du \textit{gameplay} car ceux-ci deviendront essentiels à la survie du joueur. En effet les fonctionnalités de \textit{respawn} dépendront des cristaux.
...
L'amélioration de l'historie se fera en deux partie : l'écriture des partie non encore définies de l'histoire et l'implémentation d'une partie de l'histoire dans le jeu. Une patrie de l'histoire sera réalisée sous forme de didacticiel, ce qui permettra à tout nouveau joueur de découvrir le monde et les différentes interfaces mises à sa disposition.

\subsubsection{Environnement sonore}

L'ajout de sons/musiques nous permettra de dynamiser notre jeu. L'ajout des créatures amènera à l'ajout des  sont qui leur sont associés. Ces bruits rendront les créature plus impressionnantes ou au contraire attirantes, attention à ne pas se fier aux apparences qui peuvent être trompeuses...
