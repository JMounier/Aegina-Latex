...
\textcolor{red}{Expliquer skybox: je suppose que c'est l'élément qui permet de définir le ciel, que vous avez voulu le faire évoluer pour représenter les différents moments de la journée/nuit}
...
\textcolor{red}{Mettre une copie d'écran}
...
\begin{figure}[ht]
\begin{center}\includegraphics[width=11cm]{Photo_1_premiere_ile.png}
...
\begin{figure}[ht]
\begin{center}\includegraphics[width=12cm]{environment.png}
...
\textcolor{red}{insérer copie d'écran}
...
\textcolor{red}{Il faut détailler, comment gérez vous la synchronisation, quel est le principe d'interpolation, en quoi rend-il les déplacements plus fluides ?}
...
\textcolor{red}{faites-vous une différence entre aléatoire et procédure, si non, il y en a une !}

Comme nous l'avions spécifié dans notre cahier des charges, le monde du jeu est généré de manière procédurale. Pour cette première soutenance, nous avons implémenté la génération aléatoire d'un \textit{chunk}, et la liaison des chunks entre eux. Nous avons donc au lancement d'une partie un monde composé de quatre chunks chacun relié par des ponts.
\\
Nous avons créé 2 formats de chunk différents possédant chacun une île. Sur ces îles sont posées des ancres qui correspondent a des emplacements d'objets. Cela permet lors de la génération du monde de choisir un biome aléatoirement parmi forêt, désert ou neige, puis de créer sur les ancres un objet cohérent avec le biome. Par exemple dans un biome désert nous trouverons des cactus et des rochers alors que dans un biome forêt nous trouverons des arbres et des fleurs.
\\
Nous avons donc tous les éléments pour créer des chunks, il ne restera plus qu'a faire apparaître les chunks de manière procédurale avec un agencement correct. Puis il sera aussi nécessaire de sauvegarder le monde avec les modifications que les joueurs y ont apportées. 

\textcolor{red}{différence entre chunk et île ? Il est question de 2 formats, lesquels sont-ils ?}
...
Une particularité du monde d'Aegina et la présence sur certaines de ses îles d'un cristal. Ce cristal n'est pas juste comme les autres éléments du décors tel que les arbres ou les pierres, alors que ceci ont pour but d'être récolter par les joueurs les cristaux eux sont indestructible. Dans la nature les cristaux ont une couleur grise indiquant qu'il n'ont pas été activer par un joueur, ils sont alors inactifs et n'ont aucun effet sur l'environnement qui l'entoure. Lorsqu'un joueur tente d'intéragir avec un cristal inactif une fenêtre apparait proposant au joueur d'activer le cristal en échange d'un petit cout en ressource. 

Une fois activer le cristal change de couleur pour prendre la couleur correspondant à la couleur de l'équipe du joueur (une couleur bleu lorsque tout les joueurs sont dans la même équipe) et se spécialise parmi trois catégories de cristaux différents de manière aléatoire. La première catégorie étant aussi la plus commune avec 60\% d'apparition est la catégorie des cristaux d'attaque (ou de défense ce qui revient au même), la deuxième catégorie avec 30\% d'apparition est celle des cristaux de production et enfin la dernière catégorie correspond aux cristaux de portail.

Actuellement il est impossible de savoir qu'elle est la catégorie d'un cristal dans le jeux et cela n'a que peux d'importance car pour l'instant les cristaux activer ne sont différents des cristaux inactif que par leur couleur.
...
\item[/time <value> :] détermine l'heure de la journée ;
\item[/give <id> <quantity> :] ajoute à l'inventaire du joueur la quantité passée en paramètre de l'objet dont l'identifiant est passé en paramètre ; 
\item[/nick <name> :] change le nom du personnage; 
\item[/msg <player> <message> :] envoie un message privé au joueur dont le nom du personnage est passé en  paramètre ; 
\end{description}
...
\textcolor{red}{il faut préciser les types des paramètres, sinon on ne voit pas en quoi day ne correspond pas au type de <value>}
