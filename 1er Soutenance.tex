...
Pour le moment, le monde contient quatre \textit{chunks} reliés par des ponts, chacun de ces \textit{chunks} est choisi aléatoirement parmi des \textit{chunks} préfabriqué composé d'îles, de ponts et des ancres. Par la suite un biome sélectionné aléatoirement est appliqué sur le \textit{chunk} ce qui influencera le type d'éléments qui remplacerons les ancres. Pour finir chaque ancre est donc remplacé par un élément cohérent avec le biome. Si le biome est \textit{désert} par exemple alors une ancre peut être remplacé aléatoirement par un cactus, un rocher, du sable\dots{} ou rien (c'est à dire un élément vide). Cette méthode de génération implique des milliers de possibilité pour un mondes composé seulement de quatre \textit{chunk}.

Nous avons donc tous les éléments pour créer des \textit{chunks}, il ne restera plus qu'a faire apparaître le monde de manière procédurale avec un agencement des \textit{chunks} correct. Puis il sera aussi nécessaire de sauvegarder le monde avec les modifications que les joueurs y ont apportées. 
