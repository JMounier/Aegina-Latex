...
\end{figure}
\\
Dans l'interface d'enregistrement de serveur (voir figure \ref{CreationServeur}) il nous est seulement demander de choisir une adresse ip et un nom sous laquelle l'enregistrer puis d'appuyer sur le bouton \og Create\fg{} pour enregistrer un nouveau serveur. contrairement à la création de monde on peut créer plusieurs serveur de même nom ou de même adresses IP s'en créer de conflit.
\begin{figure}[h!]
\begin{center}
...
\end{center}
\end{figure}
Notre génération de monde commençant à être longue avec la génération des îles et de leurs graphes  Nous avons rajouter un écran de chargement sous la forme d'une vidéo format MP4 tournant en boucle. Ceci permet à Unity d'utiliser toute sa capacité de calcule sans se soucier de la génération de l'écran de chargement qui est gérer par un logiciel tiers.
\\
...
\item[/effect <player (string)> <id (int)> :] affecte le joueur passé en paramètre de l'effet correspondant à l'identifiant avec une puissance choisie (la puissance est d'un si elle n'est pas spécifiée) ;
